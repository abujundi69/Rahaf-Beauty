    ...
    <section className="container-page py-10 md:py-14">
      <div className="mb-8 rounded-2xl bg-white p-6 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <button
                ...
                className="relative grid h-11 w-11 place-items-center rounded-2xl bg-ivory text-ink transition hover:bg-petal"
              ...
              {notificationsOpen ? (
                <div className={`absolute top-12 z-[100] w-[min(88vw,22rem)] rounded-2xl border border-petal bg-white p-3 text-ink shadow-soft ${isRtl ? "left-0" : "right-0"}`}>
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <p className="text-sm font-extrabold text-ink">{t("notifications")}</p>
                    <button
                      type="button"
                      onClick={markAllAsRead}
                      className="inline-flex items-center gap-1 rounded-full bg-ivory px-3 py-1.5 text-xs font-bold text-ink hover:bg-shell"
                    >
                  ...
                  <div className="max-h-80 space-y-2 overflow-y-auto">
                    {notifications.length === 0 ? (
                      <p className="rounded-2xl bg-ivory p-4 text-sm font-semibold text-ink">
                      ...
                      notifications.slice(0, 8).map((notification) => (
                        <button
                          ...
                          className={`block w-full rounded-2xl p-4 text-start text-sm transition hover:bg-petal ${
                            notification.read ? "bg-ivory text-ink" : "bg-shell text-ink"
                          }`}
                        >
        ...
        <button
          ...
          className="mt-5 inline-flex h-10 items-center gap-2 rounded-full bg-terracotta px-4 text-sm font-extrabold text-white transition hover:bg-clay lg:hidden"
        >
